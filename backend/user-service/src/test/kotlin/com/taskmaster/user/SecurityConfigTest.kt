...
class SecurityConfigTest {
    ...
    @Test
    fun `jwtAuthenticationConverter handles single role`() {
        val jwt = buildJwt(mapOf("realm_access" to mapOf("roles" to listOf("TENANT_ADMIN"))))

        val authentication = config.jwtAuthenticationConverter().convert(jwt)!!

        assertEquals(listOf("ROLE_TENANT_ADMIN"), authentication.authorities.mapNotNull { it.authority }.filter { it.startsWith("ROLE_") })
    }
    ...
    fun `issuer from unknown realm does not match any trusted realm pattern`() {
        val unknownIssuer = "http://keycloak:8180/realms/some-other-realm"
